rule MapDamage:
    ...
    shell:
        ...
        "for i in $(cat {input.species_tax_id}); do if [[ $(wc -l < {output.dir}/${{i}}.seq_ids) -ge 1 ]]; then xargs --arg-file={output.dir}/${{i}}.seq_ids samtools view -bh {input.bam} --write-index -@ {threads} -o {output.dir}/${{i}}.tax.bam; fi; done >> {log} 2>&1; "
