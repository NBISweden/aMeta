rule KrakenUniq:
    ...
    params:
        ...
        preload_size=config.get("krakenuniq_preload_size", "96G"),
    threads: 10
    log:
    ...
    shell:
        "krakenuniq --preload-size {params.preload_size} --db {params.DB} --fastq-input {input.fastq} --threads {threads} --output {output.seqs} --report-file {output.report} --gzip-compressed --only-classified-out &> {log}"
